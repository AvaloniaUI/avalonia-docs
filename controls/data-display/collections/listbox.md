...
The list height expands to fit all the items unless set specifically (using the height attribute), or set by a containing control, such as the [dock panel](/controls/layout/panels/dockpanel).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>The container panel to place items in. See [Custom panel](/docs/how-to/itemscontrol-how-to#custom-panel) for how to customize the `ItemsPanel`.</td>
    </tr>
    <tr>
...
:::info
To review the concepts behind **data template**, see [Introduction to data templates](/docs/data-templates/introduction-to-data-templates).
:::
...
The `ListBoxItem` element acts as a container for the content specified in a `ListBox.ItemTemplate` element; but it is not ever defined in the XAML, instead it is generated by _Avalonia_.
