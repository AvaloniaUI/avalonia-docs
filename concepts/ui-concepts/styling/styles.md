...
- [Styles](/concepts/ui-concepts/styling/styles) are similar to CSS styles and are usually used to style controls based on their content or purpose within the application; for example creating a style for header text blocks.

## Control themes

- [Control Themes](/concepts/ui-concepts/styling/control-themes) are similar to WPF/UWP styles and are usually used to apply a theme to controls.

## Container Queries
- [Container Queries](/concepts/ui-concepts/styling/container-queries) are collection of styles that are applied based on the size of a container. 
...
:::info
The _Avalonia UI_ **style selector syntax** is analogous to that used by CSS (cascading style sheets). For detailed reference information, see [here](/reference/styles/style-selector-syntax). 
:::

## Example

This is an example of how a style is written and applied to a control element, with a [style class](/concepts/ui-concepts/styling/style-classes) to help selection:
...
:::info
A full list of selectors can be found [here](/reference/styles/style-selector-syntax).
:::
...
:::info
For more information on setters see [here](/reference/styles/property-setters).
:::

## Nested Styles

Styles can be nested in other styles. To nest a style, simply include the child style as a child of the parent `<Style>` element, and start the selector with the [`Nesting Selector (^)`](/reference/styles/style-selector-syntax):
...
:::info
For guidance on how to use resources in you application, see [here](/docs/ui-development/resource-dictionary).
:::

## Further Information

:::info
For guidance on how to share styles by including a styles file, see [here](/docs/ui-development/styling/sharing-styles).
:::
