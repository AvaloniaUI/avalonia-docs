...
function Section({ title, children, className = "" }: SectionProps): JSX.Element {
  return (
...
function CodeBlock({ code, highlightLine = -1 }: CodeBlockProps): JSX.Element {
  return (
...
function Intro({ onStart }: IntroProps): JSX.Element {
  return (
...
function ProductionLine(): JSX.Element {
...
function ArchitectureStack(): JSX.Element {
...
function FrameWalkthrough(): JSX.Element {
...
function PlaygroundTransform(): JSX.Element {
    ...
    <div className="flex flex-col md:flex-row gap-8">
      <div className="flex-1 space-y-6">
        ...
        <div>
           ...
           <input type="range" min="0.5" max="2" step="0.1" value={scale} onChange={e => setScale(Number(e.target.value))} className="w-full accent-purple-600" />
        </div>
      ...
      <div className="flex-1 bg-slate-100 rounded-xl flex items-center justify-center h-[300px] relative overflow-hidden border border-slate-300">
        ...
        <div
          ...
          style={{
            ...
            clipPath: clip ? 'polygon(-50% -50%, 150% -50%, 150% 150%, -50% 150%)' : 'none'
          }}
        >
           Button
        </div>
...
function PlaygroundBackend(): JSX.Element {
  const [backend, setBackend] = useState<string>('skia-cpu');

  const options: Record<string, BackendOption> = {
...
function PlaygroundDirty(): JSX.Element {
...
function TextDemo(): JSX.Element {
  ...
  const getGlyphs = (str: string): Glyph[] => {
...
function Quiz({ onRestart }: QuizProps): JSX.Element {
...
export default function AvaloniaRendering(): JSX.Element {
