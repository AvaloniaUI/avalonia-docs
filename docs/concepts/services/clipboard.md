...
Two data formats are considered equal if they have the same kind and identifier.
...
A platform format represents a format compatible *only with the current platform* that the application is running on (e.g., Windows, Linux, iOS, etc.). Its identifier should be a name understood by the underlying platform. Use this format only when you need direct interoperability with the target platform and have precise knowledge of its encoding or serialization.

:::warning
Do **not** assume that a given identifier works for all platforms!  
For example, the HTML format is named `HTML format` on Windows, `text/html` on Linux and Android, and `public.html` on macOS and iOS. Verify the operating system before using a platform-specific format if your application is cross-platform.
:::

A platform format can be defined using `DataFormat.CreateBytesPlatformFormat` or `DataFormat.CreateStringPlatformFormat`, respectively using `byte[]` and `string` types. Avalonia does not perform any automatic serialization.
...
An application format is specific to your application and works with all platforms that allow custom data formats. Its identifier is restricted to ASCII letters, digits, plus dot and dash (`A`-`Z`, `a`-`z`, `0`-`9`, `.`, `-`). This identifier will not be exposed directly to the underlying platform; instead, it will be internally prefixed to avoid conflicts with platform formats.
...
An application format can be defined using `DataFormat.CreateBytesApplicationFormat` or `DataFormat.CreateStringApplicationFormat`, respectively using `byte[]` and `string` types. Avalonia does not perform any automatic serialization.
...
The `IClipboard` interface enables interacting with the system clipboard, providing functionality for setting and retrieving text, image, and custom data formats.
...
The `TryGetDataAsync` method asynchronously retrieves an `IAsyncDataTransfer` object representing the clipboard's content. If the clipboard content is empty, `null` is returned.
...
:::warning
Since the clipboard's content may change at any time, it is recommended to use the returned `IAsyncDataTransfer` instance as soon as possible; do not store it for later use. After all the necessary operations are done on the object, it must be disposed of by the caller; a `using` statement is therefore recommended.
:::

#### Extension Methods

A common clipboard usage is to retrieve data using a specific format. Several extension methods are available to support common formats. You can use those instead of `TryGetDataAsync()`:
...
Calling this method avoids accessing the underlying platform's clipboard, which can be useful when fine-grained control is needed. For most scenarios, using `TryGetDataAsync()` should be the preferred option.

In practice, this method is supported on Windows, macOS, and X11.
...
A common clipboard usage is to write a single specific format. Several extension methods are available for convenience for common formats:
...
If the Avalonia application terminates, the data won't be available anymore.
...
Instead, consider using the extension methods described below to get typed results.
:::
...
Implementing both `IAsyncDataTransfer` and `IAsyncDataTransferItem` is required to provide values for writing to the clipboard.
...
- `SetFile(IStorageItem)` sets a `IStorageItem` object for the `DataFormat.File` format.
- `SetBitmap(Bitmap)` sets a `Bitmap` image for the `DataFormat.Bitmap`.

Additionally, static factories exist to create items with a single format, a common scenario:
...
- `DataTransferItem.CreateBitmap(Bitmap)` for the `DataFormat.Bitmap`.
...
- Verify that [`DataTransfer` and `DataTransferItem`](#datatransfer--datatransferitem) are not enough for your needs.
- Ensure that `IAsyncDataTransfer.Formats` contains the formats from all its items, without duplicates.
- Be aware that `IAsyncDataTransferItem.TryGetRawAsync()` can be called from any thread and does not use the dispatcher.
- Consider also implementing `IDataTransfer` and `IDataTransferItem`, as some platforms can only access the clipboard synchronously.
