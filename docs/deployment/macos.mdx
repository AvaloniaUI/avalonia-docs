...
id: macos
title: macOS
description: Package, code sign, notarize, and distribute an Avalonia app as a macOS .app bundle, with App Store and GitHub Actions guidance.
doc-type: how-to
---
...
[Parcel](/tools/parcel/packaging-for-macos) automates macOS packaging for Avalonia apps. It handles bundle structure, `Info.plist` generation, code signing, notarization, and DMG creation from a single command. Parcel can also create macOS bundles from Windows and Linux. If you'd prefer to avoid the manual steps below, see the [Parcel setup guide](/tools/parcel/setup) to get started.
:::
        ...
        children: [
          {
            name: '_CodeSignature',
            type: 'folder',
            comment: 'stores code signing information',
            children: [
          ...
          },
          {
            name: 'MacOS',
            type: 'folder',
            comment: 'all your DLL files, etc. -- the output of dotnet publish',
            children: [
          ...
          },
          {
            ...
            type: 'folder',
            children: [
              { name: 'MyProgramIcon.icns', type: 'file', comment: 'icon file' }
            ]
          },
          { name: 'Info.plist', type: 'file', comment: 'stores information on your bundle identifier, version, etc.' },
          { name: 'embedded.provisionprofile', type: 'file', comment: 'file with signing information' }
        ]
      }
    ]
  }
...
|---|---|---|
| `CFBundleExecutable` | Must match the binary name from `dotnet publish` (your assembly name without `.dll`). | `MyApp` |
| `CFBundleName` | Display name for your application (max 15 characters). | `My App` |
| `CFBundleDisplayName` | Full display name. Only needed if `CFBundleName` exceeds 15 characters. | `My Application` |
| `CFBundleIconFile` | Name of your `.icns` icon file, including extension. | `MyApp.icns` |
| `CFBundleIdentifier` | Unique identifier in reverse-DNS format. | `com.myapp.macos` |
| `NSHighResolutionCapable` | Enables Retina display support. Set to `<true/>`. | `true` |
| `CFBundleVersion` | Internal build version number. | `1.4.2` |
| `CFBundleShortVersionString` | User-visible version string. | `1.4.2` |

If you need to register custom URL schemes or file type associations, see the [macOS platform integration guide](/docs/platform-specific-guides/macos).

For a complete list of `Info.plist` keys, see Apple's [Information Property List](https://developer.apple.com/documentation/bundleresources/information_property_list/bundle_configuration) reference.
...
2. Add your Apple ID if it isn't already listed.
3. Select your team and choose **Download Manual Profiles**. This downloads the certificate into your keychain.
4. To share the certificate with another developer, find it in Keychain Access, right-click, and choose **Export**. This generates a password-protected `.p12` file.
...
Enabling hardened runtime is done in the same step as codesigning. You have to codesign everything in the `.app` bundle under the `Contents/MacOS` folder, which is easiest to do with a script since there are a lot of files. To sign your files, you need an Apple developer account.

To notarize your app, you need a Developer ID certificate, which requires a paid Apple developer subscription. See [Obtaining a Developer ID certificate](#obtaining-a-developer-id-certificate) for instructions on how to install a certificate locally.
...
:::caution[Don't use the `--deep` flag]
Some guides suggest signing the whole bundle in a single pass with `codesign --deep`. Apple has deprecated `--deep` for signing, and it can leave nested binaries improperly signed. Sign each item explicitly instead, as the script above does, by walking the files first and signing the bundle last.
:::
...
* App Store Provision Profile. Get it for your app from the [Apple Developer provisioning profiles page](https://developer.apple.com/account/resources/profiles/list).
...
* In KeyChain you should see this certificates `3rd Party Mac Developer Installer` and `Apple Distribution`. If cert names start with another string, you may have created a wrong certificate. Try again.
...
Copy your `.app` into the Applications folder and launch it. If it launches correctly, you did everything right. If it crashes, open the Console and check the crash report.
...
- [Packaging for macOS with Parcel](/tools/parcel/packaging-for-macos) to automate bundling, signing, and notarization
- [macOS platform integration](/docs/platform-specific-guides/macos) for URL schemes, file associations, and native APIs
- [Deploying to iOS](/docs/deployment/ios) for iOS and Mac Catalyst builds from the same project
- [Publishing with Native AOT](/docs/deployment/native-aot) for ahead-of-time compiled builds
