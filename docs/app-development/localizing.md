...
You must ensure a `public` resource class is available. Otherwise, your `{x:Static}` references may fail. (Because an `internal` class still compiles.) How you generate the `Resources` class depends on the tooling you use: [Visual Studio](#visual-studio) or [other tools](#other-tools-rider-vs-code-cli).
