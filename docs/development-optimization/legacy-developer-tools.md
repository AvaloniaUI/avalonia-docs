...
The events tab can be used to track the propagation of [events](/concepts/ui-concepts/user-input/routed-events). Select the events to track in the left pane, and all events of that type will be shown in the center upper pane. Select one of these events to see the event route.
