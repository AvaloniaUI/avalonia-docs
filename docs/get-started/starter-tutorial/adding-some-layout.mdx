...
doc-type: tutorial
video:
  src: https://youtu.be/eFGHa8BIxhY
  title: How to Build Layouts in Avalonia 12 — Rows, Columns & Spacing
---
...
For a full list of Avalonia layout controls, go to the Layout tab of the [Controls reference](/controls).

## Inserting a stack panel

You can use the [`StackPanel`](/controls/layout/panels/stackpanel) layout control to place some text above the button.
...
2. Add a [`TextBlock`](/api/avalonia/controls/textblock) above the Button. (You may recall the `TextBlock` control from the default **MainWindow.axaml**, which prints text in the window.) Set the attributes of the `TextBlock` as follows:
...
:::note
By default, [`StackPanel`](/controls/layout/panels/stackpanel) arranges elements in a vertical stack. You can change this to horizontal by setting the `Orientation` attribute to `Horizontal`.
:::

## Inserting a grid

Next, add a [`Grid`](/api/avalonia/controls/grid) layout control to the temperature converter app. `Grid` creates cells in rows and columns, into which you can place more controls.
...
:::tip
In this example, `Grid` goes inside `StackPanel` so that the entire grid's contents appear between the "Temperature Converter" text box and the **Calculate** button. There are many options for combining layout controls to get the exact UI design you need. See [Choosing a layout panel](/docs/layout/choosing-a-layout-panel) for more information on when to use different types of layout control.
:::
